...
!integer, parameter, public :: rt = selected_real_kind(15)   ! REAL*8
!end module kinds
!
!MODULE module_constants
!use kinds, only   :  rt
!real(rt), parameter :: pi       = 3.14159265358979323846d0
!real(rt)            :: lon_pol  = 0.d0   ! give a value for pole longitude
...
! YGYU: forward substittion 
!
!  L * Y = b
!      L is lower triangle,  L(i,i) .NE. 0
...
!  |                 |
!
SUBROUTINE forwardsb_LY_b(a,n,np,b)
      INTEGER n,np
      REAL*8 a(np,np),b(np)
      INTEGER i,j
      REAL*8 sum
...
! a3:      k
!
subroutine matrix_det_3by3 ( A, A_det, n, np )
  implicit none
  integer :: n, np
  real*8, intent(in ) :: a(np, np)
  real*8, intent(out) :: A_det
  real*8  :: s
  integer :: i, j, k
  !
  !- stop if not 3 by 3
  ...
  do i=1, 3
     ...
     s=  s+ A(1,i)*(A(2,j)*A(3,k)-A(3,j)*A(2,k))
  enddo
  A_det= s
  return
end subroutine matrix_det_3by3
...
! xnm = sqrt( x dot x )
  ...
  xnm = dsqrt( s )
  return
end subroutine X_2norm


subroutine normalize_vec( x, n )
! xnm = sqrt( x dot x )
  ...
  enddo
  if (s .LE. 0.d0) then
     write(6, '(//,2x,a,//)')  'Error in subroutine normalize_vec: zero norm'
     STOP 'zero norm in subroutine normalize_vec'
  endif
  do i = 1, n
     x(i)= x(i)/sqrt(s)
  enddo
...
subroutine area_plane_triangle (V1, V2, V3, area, n)
!  area = norm ( X cross Y )
!  X= V2-V1;  Y=V3-V1
  integer, intent(in)  :: n
  ...
  Z(2) = X(3)*Y(1) - X(1)*Y(3)
  Z(3) = X(1)*Y(2) - X(2)*Y(1)
  area = dsqrt (Z(1)*Z(1) + Z(2)*Z(2) + Z(3)*Z(3))
  return
end subroutine area_plane_triangle
...
subroutine  XY_cross_unit( X, Y, Z, n)
  ...
  Z(2) = X(3)*Y(1) - X(1)*Y(3)
  Z(3) = X(1)*Y(2) - X(2)*Y(1)
  s    = sqrt(Z(1)*Z(1)+Z(2)*Z(2)+Z(3)*Z(3))
  if (s .LE. 1.d-9) STOP 's=0, sub: XY_cross_unit'
  Z(1) = Z(1)/s
  Z(2) = Z(2)/s
  Z(3) = Z(3)/s
  return
end subroutine XY_cross_unit
...
function combine_num(n, m)
   integer :: n, m
   integer :: combine_num    ! combine=C_n^m
   ! local
   integer :: i, kn, km, knm
...
! This is for the unlikely event that rmax exceed r(mesh)
        !yfit(i)=0.d0        ! give zero for atomic physics
         yfit(i)=yspline(nspline)
 10     continue
      end do
...
subroutine arc_len (P1, P2, s, ndim)
  ...
  ! 
!  s = asin( sqrt( sin((p1(2)-p2(2))/2.d0)**2.d0 + cos(p1(2))*cos(p2(2))*   &
!                         sin((p1(1)-p2(1))/2.d0)**2.d0 ) ) * 2.d0
!
  s = asin( sqrt( sin((p1(2)-p2(2))/2.d0)*sin((p1(2)-p2(2))/2.d0) + &
       cos(p1(2))*cos(p2(2))*sin((p1(1)-p2(1))/2.d0)*sin((p1(1)-p2(1))/2.d0)))*2.d0
!
  return
...
!-- input P1, P2:  (X, Y, Z)
subroutine great_circ_dis_car (P1, P2, dis, ndim, radius)
  use kinds, only: rt
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in) :: P1(ndim), P2(ndim) ! pt1_llr(3), pt2_llr(3), lam, theta, r
  real(rt),intent(in) :: radius
  real(rt),intent(out):: dis                !  arc_len
  real(rt)  :: s, t, u
  !
  !   cos(X) = (n1 \dot n2) /  |n1|*|n2|
  !
  s= P1(1)*P2(1) + P1(2)*P2(2) + P1(3)*P2(3)    !  A dot B
  t= (P1(1)*P1(1) + P1(2)*P1(2) + P1(3)*P1(3)) * (P2(1)*P2(1) + P2(2)*P2(2) + P2(3)*P2(3))
  if (t.le.0.d0) then
     dis=0.d0
  else
     u= s/sqrt(t)
  ...
  return
end subroutine great_circ_dis_car


!-- input P1, P2:  (X, Y, Z)
subroutine arc_len_vector_version (P1, P2, dis, ndim)
  use kinds, only: rt
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in ):: P1(ndim), P2(ndim) ! pt1_llr(3), pt2_llr(3), lam, theta, r
  real(rt),intent(out):: dis     !  arc_len
  real(rt)  :: V1(ndim), s, t, u 
  !
  !   cos(X) = (n1 \dot n2) /  |n1|*|n2|
  !
  s= P1(1)*P2(1) + P1(2)*P2(2) + P1(3)*P2(3)    !  A dot B
  ...
  return
end subroutine arc_len_vector_version
...
subroutine basis_between_sph_car (P_sph, basis, invbasis, ndim)
  ...
  real(rt),intent(out):: basis(3,3), invbasis(3,3)
  real(rt)  :: lon, lat, r
  integer   :: i, j
  !
  ...
  !   z = r sin \theta
  !
  if (ndim.NE.3) STOP ' nidm .NE. 3, error in basis_between_sph_car'
  ...
  do i=1, 3
     do j=1, 3
        invbasis(i,j)=basis(j,i)
     enddo
  enddo
  !
  return
end subroutine basis_between_sph_car

!
!----   Not right  -----
!
!
!subroutine basis_from_sph_2_car (P_sph, basis, invbasis, ndim)
!  use kinds, only: rt
...
!  !
!  lon=P_sph(1); lat=P_sph(2); r=P_sph(3)
!  if ( abs(cos(lat)).LT.1.d-8) then  
!     ! singularity,  choose  \vec lam = x hat,  \vec theta = y hat
!     !lon=-pi/2.d0; lat=pi/2.d0
!     lon=lon_pol
!  endif
!  !
!  basis(1,1)= - sin(lon)               ! \vec lambda \dot x  =  - x/rho,  rho=sqrt(x^2+y^2)
...
!  else
!     ! polar singularity
!     ! singularity  define \lambda= \vec x,  \theta= \vec y
!     !    vi = \xi \dot polar
!     basis(1:3,1:3)=0.d0
!     if (z.gt.0.d0) then
!        basis(1,1)= 1.d0    ! north pole
!        basis(2,2)= 1.d0
!        basis(3,3)= 1.d0
!     else
!        basis(1,1)= 1.d0    !  south pole
!        basis(2,2)=-1.d0    !  \theta = - \vec y 
!        basis(3,3)=-1.d0    !  \r     = - z
!     endif
!  endif
!  return
...
subroutine vect_from_sph_2_car(P_sph, P_car, ndim)
  ...
  !
  lon=P_sph(1); lat=P_sph(2); r=P_sph(3)
  x= cos(lat)*cos(lon)                 ! assume r=1.d0
  y= cos(lat)*sin(lon)
  z= sin(lat)
...
subroutine vect_from_car_2_sph (P_car, P_sph, ndim)
  ...
  lat= asin(z/r)
  !
  if (rho/r .GT. 0.d0) then
     if ( y.GE.0.d0) then
        lon = acos( x/rho )
     else
        lon = 2.d0*pi - acos( x/rho )        
     endif
  else
     lon = lon_pol   ! N/S polar singularity, give a constant longitude
!     STOP 'sub: vect_from_car_2_sph : North/South polar coordinate undefined'
  endif
  P_sph(1)=lon
  P_sph(2)=lat
  P_sph(3)=r      ! donot worry, after this, I reset r=1 in the calling subroutine
  return
end subroutine vect_from_car_2_sph


!-     Input:  car coord. P1, P2 in correct direction Down --> Up
!
!----  T: P2-P1
!----  N: outward direction =  P2 \cross P1
!
subroutine arc_mid_ntrvec_car (P1, P2, Pmid_car, Pmid_sph, Vn_car, Vt_car, Vr_car, ndim)
  use kinds, only: rt
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in ):: P1(ndim), P2(ndim)     !  car coord.
  real(rt),intent(out):: Pmid_car(3), Pmid_sph(3)
  real(rt),intent(out):: Vn_car(3), Vt_car(3), Vr_car(3)  ! N_vec, T_vec, R_vec
  real(rt)  :: s1, s2, s3
  real(rt)  :: T(3), R(3), N(3)
  integer   :: i
  !
  !
  !-- Get absolute vector, then normalize to unit
  !
  do i=1,3
     T(i)= P2(i)-P1(i)
     R(i)= P2(i)+P1(i)
  enddo
  call XY_cross (P2, P1, N, 3)
  s1 = T(1)*T(1) + T(2)*T(2) + T(3)*T(3)
  s2 = R(1)*R(1) + R(2)*R(2) + R(3)*R(3)
  s3=  N(1)*N(1) + N(2)*N(2) + N(3)*N(3)
  !
  !-- did not check close to zero
  !
  do i=1,3
     Vt_car(i)= T(i)/sqrt(s1)
     Vr_car(i)= R(i)/sqrt(s2)
     Vn_car(i)= N(i)/sqrt(s3)
  enddo
  Pmid_car(1:3)=Vr_car(1:3)
  call vect_from_car_2_sph (Pmid_car, Pmid_sph, ndim)
  return
end subroutine arc_mid_ntrvec_car
...
subroutine arc_mid_ntrvec (P1_sph, P2_sph, Pmid_car, Pmid_sph, Vn_car, Vt_car, Vr_car, ndim)
  ...
  real(rt),intent(out):: Pmid_car(3), Pmid_sph(3)
  real(rt),intent(out):: Vn_car(3), Vt_car(3), Vr_car(3)  ! N_vec, T_vec, R_vec
  real(rt)  :: lon1, lon2, lat1, lat2, s, t, z
  real(rt)  :: dR
  real(rt)  :: P1_car(3), P2_car(3), rvec_car(3)
  integer   :: i
  !
  dR= abs(P1_sph(3) - P2_sph(3))
  if (dR .GT. 1.d-8) STOP 'P1_sph, P2_sph have different radius'
  ...
  !
  do i=1,3
     Vt_car(i)=(P2_car(i)-P1_car(i))
     Pmid_car(i)= 0.5d0*(P2_car(i)+P1_car(i))
  enddo
  call XY_cross_unit (P2_car, P1_car, Vn_car, 3)
  !
  call X_2norm (Vn_car, s, 3)
  call X_2norm (Vt_car, t, 3)
  call X_2norm (Pmid_car, z, 3)
  do i=1,3
     Vn_car(i)= Vn_car(i)/s
     Vt_car(i)= Vt_car(i)/t
     Vr_car(i)= Pmid_car(i)/z
  enddo
  call vect_from_car_2_sph (Pmid_car, Pmid_sph, ndim)
  return
end subroutine 
...
subroutine arc_mid_pt (P1_sph, P2_sph, Pmid_car, Pmid_sph, ndim)
  use kinds, only: rt
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in ):: P1_sph(ndim), P2_sph(ndim) ! pt1_llr(3)  lam, theta, r
  ...
  call X_2norm (Pmid_car, z, 3)
  do i=1,3
     Pmid_car(i)= Pmid_car(i)/z
  enddo
  call vect_from_car_2_sph (Pmid_car, Pmid_sph, ndim)
  return
end subroutine arc_mid_pt
...
!----  N: outward direction =  P2 \cross P1
!
subroutine arc_innerpt_ntrvec (P1_sph, P2_sph, P_inner_sph, Vn_car, Vt_car, Vr_car, ndim)
  use kinds, only: rt
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in ):: P1_sph(ndim), P2_sph(ndim)  ! pt1_llr(3)  lam, theta, r
  real(rt),intent(in ):: P_inner_sph(3)
  real(rt),intent(out):: Vn_car(3), Vt_car(3), Vr_car(3)  ! N_vec, T_vec, R_vec
  real(rt)  :: lon1, lon2, lon3, lat1, lat2, lat3, s, t, z
  real(rt)  :: dR
  real(rt)  :: P1_car(3), P2_car(3), P3_car(3)
  integer   :: i
  !
  dR= abs(P1_sph(3) - P2_sph(3))
  if (dR .GT. 1.d-8) STOP 'P1_sph, P2_sph have different radius'
  lon1=P1_sph(1); lat1=P1_sph(2)
  lon2=P2_sph(1); lat2=P2_sph(2)
  lon3=P_inner_sph(1); lat3=P_inner_sph(2)
  P1_car(1)= cos(lat1)*cos(lon1)   ! x
  P1_car(2)= cos(lat1)*sin(lon1)   ! y
  P1_car(3)= sin(lat1)             ! z
  P2_car(1)= cos(lat2)*cos(lon2)   ! x
  P2_car(2)= cos(lat2)*sin(lon2)   ! y
  P2_car(3)= sin(lat2)             ! z
  P3_car(1)= cos(lat3)*cos(lon3)   ! x
  P3_car(2)= cos(lat3)*sin(lon3)   ! y
  P3_car(3)= sin(lat3)             ! z
  !
  ! s1: Nvec
  call XY_cross_unit (P2_car, P1_car, Vn_car, 3)
  ! s2: Rvec
  Vr_car(1:3)= P3_car(1:3)
  ! s3: Tvec = R \cross N
  call XY_cross_unit (Vr_car, Vn_car, Vt_car, 3)
  ! s4: norm
  call X_2norm (Vn_car, s, 3)
  call X_2norm (Vt_car, t, 3)
  call X_2norm (Vr_car, z, 3)
  ! s5: normalize
  do i=1,3
     Vn_car(i)= Vn_car(i)/s
     Vt_car(i)= Vt_car(i)/t
  ...
  enddo
  return
end subroutine arc_innerpt_ntrvec



!
!----  N: outward direction =  tangent for mid point = (P2 - P1)
!----  R:  R for mid pts of (P1 + P2)
!      T:  R  \cross  N
!                    V_up
!                   |
!  (ip) P1 _______  | ________ P2 (jp)
!                   |
!                    V_dn
!
subroutine arc_CVTpt_ntrvec (P1_sph, P2_sph, Vn_car, Vt_car, Vr_car, ndim)
  use kinds, only: rt
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in ):: P1_sph(ndim), P2_sph(ndim)  ! pt1_llr(3)  lam, theta, r
  ...
  real(rt)  :: lon1, lon2, lat1, lat2, s, t, z
  real(rt)  :: dR
  real(rt)  :: P1_car(3), P2_car(3), P3_car(3), Pmid_car(3)
  integer   :: i
  !
  dR= abs(P1_sph(3) - P2_sph(3))
  if (dR .GT. 1.d-8) STOP 'P1_sph, P2_sph have different radius'
  ...
  !
  do i=1,3
     Pmid_car(i)= 0.5d0 * (P1_car(i) + P2_car(i))     ! Rvec
     P3_car(i)  = P2_car(i) - P1_car(i)              ! N
  enddo
  !
  ! s1: Nvec
  Vn_car(1:3)= P3_car(1:3)  
  !
  ! s2: Rvec
  Vr_car(1:3)= Pmid_car(1:3)
  !
  ! s3: Tvec = R \cross N
  call XY_cross_unit (Vr_car, Vn_car, Vt_car, 3)
  !
  ...
  enddo
  return
end subroutine arc_CVTpt_ntrvec
...
!          \
!           p2
!!subroutine  spherical_angle(P1_sph, P2_sph, P3_sph, angle1, ndim)
!!
!
!
!
subroutine area_sph_triangle (P1_sph, P2_sph, P3_sph, area, ndim)
  ...
  real(rt)  :: V1(3), V2(3), V3(3), A(3,3)  ! cartesian of P1_sph, P2_sph, etc.
  real(rt)  :: n1(3), n2(3), angle(3), n1n2norm2, n1dn2
  real(rt)  :: R, s, x
  integer   :: i, j, k
  !
  ...
  !           O
  !
  R= P1_sph(3)   ! assume this radius is absolute 1
  if (abs(P2_sph(3)-R).GT.1.d-10 .OR. abs(P3_sph(3)-R).GT.1.d-10 ) then
     write(6, 101) 'P2_sph(3)-R=', P2_sph(3)-R
     write(6, 101) 'P3_sph(3)-R=', P3_sph(3)-R
     STOP 'P1,P2,P3 not on sphere, sub: area_sph_triangle'
  endif
  ...
  A(1:3,3)=V3(1:3)
  !
  !-  loop point i, find angle <j,i,k> centered on i
  s= 0.d0
  do i=1, 3
     !
     !
     ! n1= ei cross ej, e2= ei cross ek, 
     ! cos( <jik> ) = n1 dot n2 /  [norm(n1) * norm(n2)]
     !
     j= mod(i,3)+1; k=mod(i+1,3)+1   ! ip1, ip2, cyclic continue
     call XY_cross (A(1,i), A(1,j), n1, ndim)
     call XY_cross (A(1,i), A(1,k), n2, ndim)
     !!n1_norm2= n1(1)*n1(1) + n1(2)*n1(2) + n1(3)*n1(3) 
     !!n2_norm2= n2(1)*n2(1) + n2(2)*n2(2) + n2(3)*n2(3) 

     n1n2norm2= (n1(1)*n1(1) + n1(2)*n1(2) + n1(3)*n1(3)) &
          * (n2(1)*n2(1) + n2(2)*n2(2) + n2(3)*n2(3))
     n1dn2   = n1(1)*n2(1) + n1(2)*n2(2) + n1(3)*n2(3) 
     if (n1n2norm2 .LE. 0.d0) then
        angle(i)= 0.d0
     else
        x= n1dn2/sqrt(n1n2norm2)
        if ( abs(x) .GT. 1.d0) then  !  this is impossible 
           angle(i)= 2.d0 * atan(1.d0)  ! pi/2 
           write(6,101) 'I got pi/2 spherical angle, wrong!'
        else
           angle(i)= acos(x)
        endif
     endif
     s= s+angle(i)
  enddo
!!  area= (s-pi)*ae*ae     !  Excess * R^2, Excess = A1+A22+A3-pi
  area= (s-pi)       !  Excess * R^2, Excess = A1+A22+A3-pi
  return
101 format (2x,a15, 2x, 4f15.10)
end subroutine area_sph_triangle



subroutine area_sph_triangle_car (P1, P2, P3, area, ndim, radius)
  use kinds, only: rt
  use module_constants, only : pi
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in ):: P1(ndim), P2(ndim), P3(ndim), radius
  real(rt),intent(out):: area
  real(rt)  :: A(3,3)  
  real(rt)  :: n1(3), n2(3), angle(3), n1n2norm2, n1dn2
  real(rt)  :: R, s, x
  ...
  do i=1, 3
     ...
     s= s+angle(i)
  enddo
  area= (s-pi)*radius*radius     !  Excess * R^2, Excess = A1+A22+A3-pi
  return
101 format (2x,a15, 2x, 4f15.10)
end subroutine area_sph_triangle_car


!!!!
!!!!Method-2:  too many tan, atan
!!!!
!!subroutine area_sph_triangle_tan (P1_sph, P2_sph, P3_sph, area, ndim)
!!  use kinds, only: rt
!!  implicit none
!!  integer ,intent(in) :: ndim
!!  real(rt),intent(in ):: P1_sph(ndim), P2_sph(ndim), P3_sph(ndim)
...
!!  return
!!101 format (2x,a15, 2x, 4f15.10)
!!end subroutine area_sph_triangle_tan
!!
...
!-- car coord
!
subroutine area_sph_triangle_by_3vector (P1, P2, P3, area, ndim)
  use kinds, only: rt
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in ):: P1(ndim), P2(ndim), P3(ndim)
  real(rt),intent(out):: area
  real(rt)  :: A(3), B(3), C(3), M(3,3)
  real(rt)  :: s, AdB, BdC, CdA, Adet, R
  integer   :: i
  !
  !  Area = S
  !  sin(S/2) = det( M(A,B,C) ) / sqrt( 2 * (1 + A d B) *  (1 + B d C) *  (1 + C d A) )
  !
  !     P1         P2 
  !       \       /
  !        \     /
  ...
  !          \ /______  P3_
  !
  if (ndim.NE.3) stop 'nidm .NE. 3, stop at area_sph_triangle_by_3vector'
  M(1:3,1)= P1(1:3)
  M(1:3,2)= P2(1:3)
  M(1:3,3)= P3(1:3)
  A(1:3)= P1(1:3)
  B(1:3)= P2(1:3)
  C(1:3)= P3(1:3)
  call XY_dot (A, B, ndim, ndim, AdB)
  call XY_dot (B, C, ndim, ndim, BdC)
  call XY_dot (C, A, ndim, ndim, CdA)
  !
  !- det
  !
  call  matrix_det_3by3 ( M, Adet, ndim, ndim )
  s= Adet / sqrt( 2.d0 * ( 1.d0 + AdB )* ( 1.d0 + BdC )* ( 1.d0 + CdA ) )
  area= 2.d0 * asin(s)
  return
101 format (2x,a15, 2x, 4f15.10)
end subroutine area_sph_triangle_by_3vector


!
!-- dangerous  Matrix_det is from ludcmp ... dead loop
!
!subroutine area_sph_triangle_car (P1_car, P2_car, P3_car, area, ndim, radius)
!  use kinds, only: rt
!  implicit none
!  integer ,intent(in) :: ndim
!  real(rt),intent(in ):: P1_car(ndim), P2_car(ndim), P3_car(ndim), radius
!  real(rt),intent(out):: area
!  real(rt)  :: A(3), B(3), C(3), Matrix(3,3)
!  real(rt)  :: s, AdB, BdC, CdA, Adet, R
!  integer   :: i
!  !
!  !  Area = S
...
!  call XY_dot (B, C, ndim, ndim, BdC)
!  call XY_dot (C, A, ndim, ndim, CdA)
!  call  matrix_det ( Matrix, Adet, ndim, ndim )
!  s= Adet / sqrt( 2.d0 * ( 1.d0 + AdB )* ( 1.d0 + BdC )* ( 1.d0 + CdA ) )
!  area= 2.d0 * asin(s) * radius * radius
!  return
!101 format (2x,a15, 2x, 4f15.10)
!end subroutine area_sph_triangle_car
!
...
!
!-- this is also an inaccurate implementation, impractical, low accuracy
!
subroutine angle_sph_triangle (P1_sph, P2_sph, P3_sph, angle, ndim)
  use kinds, only: rt
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in ):: P1_sph(ndim), P2_sph(ndim), P3_sph(ndim)
  real(rt),intent(out):: angle(ndim)
  real(rt)  :: V1(3), V2(3), V3(3)      ! cartesian of P1_sph, P2_sph, etc.
  real(rt)  :: R, a, b, c, s, E
  real(rt)  :: s1, s2, s3               ! dot product
  integer   :: i
  ...
  !          \ /______  P3_
  !
  if (ndim .NE. 3) STOP 'ndim .NE. 3, stop'
  R= P1_sph(3)
  if (abs(P2_sph(3)-R).GT.1.d-10 .OR. abs(P3_sph(3)-R).GT.1.d-10 ) then
     write(6, 101) 'P2_sph(3)-R=', P2_sph(3)-R
     write(6, 101) 'P3_sph(3)-R=', P3_sph(3)-R
     STOP 'P1,P2,P3 not on sphere, sub: angle_sph_triangle'
  endif
  call vect_from_sph_2_car (P1_sph, V1, ndim)
  call vect_from_sph_2_car (P2_sph, V2, ndim)
  call vect_from_sph_2_car (P3_sph, V3, ndim)
  ...
  c= R* acos(s3/R/R)
  s= 0.5d0*(a+b+c)
  angle(1)=2.d0*acos(sqrt( sin(s)*sin(s-a)/sin(b)/sin(c) ))
  angle(2)=2.d0*acos(sqrt( sin(s)*sin(s-b)/sin(c)/sin(a) ))
  angle(3)=2.d0*acos(sqrt( sin(s)*sin(s-c)/sin(a)/sin(b) ))
  return
101 format (2x,a15, 2x, 4f15.10)
end subroutine angle_sph_triangle



subroutine angle_sph_triangle_car (P1, P2, P3, angle, ndim)
  use kinds, only: rt
  use module_constants, only : pi
  implicit none
  integer ,intent(in) :: ndim
  ...
  real(rt),intent(out):: angle(ndim)
  real(rt)  :: A(3,3)  
  real(rt)  :: n1(3), n2(3), n1n2norm2, n1dn2
  real(rt)  :: R, s, x
  integer   :: i, j, k
  !
  A(1:3,1)=P1(1:3)
  ...
  return
101 format (2x,a15, 2x, 4f15.10)
end subroutine angle_sph_triangle_car



subroutine gc_sph_triangle (P1_sph, P2_sph, P3_sph, gc_sph, gc_car, ndim)
  use kinds, only: rt
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in ):: P1_sph(ndim), P2_sph(ndim), P3_sph(ndim)
  real(rt),intent(out):: gc_sph(ndim), gc_car(ndim)
  real(rt)  :: V1(3), V2(3), V3(3)      ! cartesian of P1_sph, P2_sph, etc.
  real(rt)  :: U1(3), U2(3), U3(3)
  real(rt)  :: R, a, b, c, s, E
  real(rt)  :: s1, s2, s3               ! dot product
  real(rt)  :: t1, t2, t3
  integer   :: i
  !
  !  spherical triangle formula from JE Brock's paper in 1970s
  !
  !  Area = R^2 * Exess,  Excess = (A+B+C)-pi
  !  tan(E/4) = sqrt [ tan s/2 * tan( (s-a)/2 ) * tan( (s-b)/2 ) * tan( (s-c)/2 ) ]
  !  s= (a+b+c)/2
  ...
  if (abs(P2_sph(3)-R).GT.1.d-10 .OR. abs(P3_sph(3)-R).GT.1.d-10 ) then
     write(6, 101) 'P2_sph(3)-R=', P2_sph(3)-R
     write(6, 101) 'P3_sph(3)-R=', P3_sph(3)-R
     STOP 'P1,P2,P3 not on sphere, sub: gc_sph_triangle'
  endif
  if (abs(R-1.d0).GT.1.d-6) STOP 'radius is not 1'
  call vect_from_sph_2_car (P1_sph, V1, ndim)
  call vect_from_sph_2_car (P2_sph, V2, ndim)
  call vect_from_sph_2_car (P3_sph, V3, ndim)
  call XY_dot (V2, V3, ndim, ndim, s1)
  ...
  E= 4.d0 * atan(sqrt(tan(0.5d0*s)*tan(0.5d0*(s-a))*tan(0.5d0*(s-b))*tan(0.5d0*(s-c))))
  !
  call XY_cross (V1, V2, U3, 3)
  call XY_cross (V2, V3, U1, 3)
  call XY_cross (V3, V1, U2, 3)
  call X_2norm  (U1, t1, 3)
  call X_2norm  (U2, t2, 3)
  call X_2norm  (U3, t3, 3)
  do i=1,3
     gc_car(i)= (U3(i)/t3*c + U1(i)/t1*a + U2(i)/t2*b)/(2.d0*E)
  enddo
  !!write(6, 101) 'gc_car(1:3)=',  gc_car(1:3)
  call normalize_vec(gc_car, ndim)
  call vect_from_car_2_sph (gc_car, gc_sph, ndim)
  gc_sph(3)=1.d0           ! only use r=1
  !
  return
101 format (2x,a15, 2x, 4f15.8)
end subroutine gc_sph_triangle
...
subroutine center_wt_sph_triangle (P1_sph, P2_sph, P3_sph, center_sph, center_car, wt, ndim)
  ...
  real(rt),intent(out):: wt(3)
  !
  real(rt)  :: V1(3), V2(3), V3(3)     ! cartesian of P1_sph, P2_sph, etc.
  real(rt)  :: T1(3), T2(3), Rvec(3)   ! cartesian of in-plane vector
  real(rt)  :: Rvec1(3), Rvec2(3), Rvec3(3)   ! cartesian three vector
  real(rt)  :: s, s1, s2, s3           ! area
  real(rt)  :: R
  integer   :: i, j
  !
  !  Area = R^2 * Exess,  Excess = (A+B+C)-pi
  ...
  ! 
  !
  !-- note: P1, P2, P3 be counter clockwise, otherwise opposite direction
  !
  R= P1_sph(3)
  if (abs(P2_sph(3)-R).GT.1.d-8 .OR. abs(P3_sph(3)-R).GT.1.d-8 ) then
     write(6, 101) 'P2_sph(3)-R=', P2_sph(3)-R
     write(6, 101) 'P3_sph(3)-R=', P3_sph(3)-R
     STOP 'P1,P2,P3 not on sphere, sub: center_wt_sph_triangle'
  endif
  call vect_from_sph_2_car (P1_sph, V1, ndim)
  call vect_from_sph_2_car (P2_sph, V2, ndim)
  call vect_from_sph_2_car (P3_sph, V3, ndim)
...
!  !
!  !-- to ensure the unique corner for the hexagon, symmetrize the Rvec
!  !
!  do i=1, 3
!     T1(i)= V2(i)-V1(i)
!     T2(i)= V3(i)-V1(i)
!  enddo
!  call XY_cross_unit (T1, T2, Rvec1, 3)     ! unit vector with r=1
!  do i=1, 3
!     T1(i)= V3(i)-V2(i)
!     T2(i)= V1(i)-V2(i)
!  enddo
!  call XY_cross_unit (T1, T2, Rvec2, 3)     ! unit vector with r=1
!  do i=1, 3
!     T1(i)= V1(i)-V3(i)
!     T2(i)= V2(i)-V3(i)
!  enddo
!  call XY_cross_unit (T1, T2, Rvec3, 3)     ! unit vector with r=1
!
!  !-- check no error in the directions, a dot b
!  call XY_dot (Rvec1, Rvec2, 3, 3, s1)
!  call XY_dot (Rvec1, Rvec3, 3, 3, s2)
!  write(12,'(2x,a,2f20.16)') 'inn(R1,R2), inn(R1,R3)', s1, s2
!  if (s1.LT.0.d0 .OR. s2.LT.0.d0) STOP 'ip,jp,kp center wrong!'
!
!  s=0.d0
!  do j=1, 3
!     Rvec(j)=(Rvec1(j)+Rvec2(j)+Rvec3(j))/3.d0
!     s=s+Rvec(j)*Rvec(j)
!  enddo
!  do j=1, 3
!     Rvec(j)=Rvec(j)/sqrt(s)
!  enddo
!  !--method 1
!  !  call XY_cross_unit (T1, T2, Rvec, 3)
!  center_car(1:3)= Rvec(1:3)
!  call vect_from_car_2_sph (center_car, center_sph, ndim)
!  Rvec(1:3)=  center_sph(1:3)


!-- test using gravity center, centroid
  s=0.d0
  do j=1, 3
     Rvec(j)=(V1(j)+V2(j)+V3(j))/3.d0
     s=s+Rvec(j)*Rvec(j)
  enddo
  do j=1, 3
     Rvec(j)=Rvec(j)/sqrt(s)
  ...
  !
  !
  call area_sph_triangle (P2_sph, P3_sph, Rvec, s1, ndim)
  call area_sph_triangle (P3_sph, P1_sph, Rvec, s2, ndim)
  call area_sph_triangle (P1_sph, P2_sph, Rvec, s3, ndim)
  s= s1+s2+s3
  wt(1)=s1/s 
  wt(2)=s2/s
  wt(3)=s3/s
...
!  call vect_from_car_2_sph (center_car, center_sph, ndim)
!  Rvec(1:3)=  center_sph(1:3)
!  center_car(1:3)= center_car(1:3)/Rvec(3)
!  Rvec(3)=1.d0
!  center_sph(3)=1.d0   ! I only need (lam, theta, r=1)


end subroutine center_wt_sph_triangle



subroutine wt_sph_triangle (P1_sph, P2_sph, P3_sph, center_sph, wt, ndim)
  use kinds, only: rt
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in) :: P1_sph(ndim), P2_sph(ndim), P3_sph(ndim)
  real(rt),intent(in) :: center_sph(ndim)
  real(rt),intent(out):: wt(3)
  !
  real(rt)  :: Rvec(3)      ! spherical coord 
  real(rt)  :: s, s1, s2, s3           ! area
  !
  !  Area = R^2 * Exess,  Excess = (A+B+C)-pi
  !  tan(E/4) = sqrt [ tan s/2 * tan( (s-a)/2 ) * tan( (s-b)/2 ) * tan( (s-c)/2 ) ]
  ...
  wt(3)=s3/s
  return
end subroutine wt_sph_triangle



!
! input: P1_car in RHS rule
!
subroutine wt_sph_triangle_car (P1, P2, P3, center, wt, ndim)
  use kinds, only: rt
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in) :: P1(ndim), P2(ndim), P3(ndim)
  real(rt),intent(in) :: center(ndim)
  real(rt),intent(out):: wt(3)
  !
  real(rt)  :: Rvec(3)      ! spherical coord 
  real(rt)  :: s, s1, s2, s3           ! area
  ...
  !
  Rvec(1:3)=  center(1:3)
  call area_sph_triangle_by_3vector (P2, P3, Rvec, s1, ndim)
  call area_sph_triangle_by_3vector (P3, P1, Rvec, s2, ndim)
  call area_sph_triangle_by_3vector (P1, P2, Rvec, s3, ndim)
  s= s1+s2+s3
  wt(1)=s1/s 
  wt(2)=s2/s
  wt(3)=s3/s
  return
end subroutine wt_sph_triangle_car



subroutine two_great_circ_intersection (P1_sph, P2_sph, P3_sph, P4_sph, intersection_sph, ndim)
  use kinds, only: rt
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in) :: P1_sph(ndim), P2_sph(ndim), P3_sph(ndim), P4_sph(ndim)
  real(rt),intent(out):: intersection_sph(ndim)
  !
  ! cartesian coord
  real(rt)  ::  V1(3), V2(3), V3(3), V4(3)
  real(rt)  ::  V1n(3), V2n(3), VX(3)
  real(rt)  ::  a, b, c, d
  real(rt)  ::  s1, s2, sa, sb, sc, sd
  real(rt)  ::  tol
  logical   ::  positive
  !
  !     P1        P3
  ...
  !
  !  Algorithm:
  !   plane_1:   normal to P1 ^ P2  =  V1n 
  !   plane_2:   normal to P3 ^ P4  =  V2n
  !   intersection line:   \vec X =  V1n  \cross  V2n
  !   If ( VX .dot. 4 PTS < 0 ),  -VX
  !
  !   Warning: if two segments meet at the end, the cos(X \dot Y) > 0 fails. 
  !   Currently I am working with bisection of small arc < pi/2
  !   Use arc length < pi to judge inside the arc
  !
  !
  if (ndim.NE.3) STOP ' nidm .NE. 3, error in basis_between_sph_car '
  tol= 1.d-8
  ...
  call vect_from_sph_2_car (P2_sph, V2, ndim)
  call vect_from_sph_2_car (P3_sph, V3, ndim)
  call vect_from_sph_2_car (P4_sph, V4, ndim)
  call XY_cross (V1, V2, V1n, ndim)
  call XY_cross (V3, V4, V2n, ndim)
  call XY_cross_unit (V1n, V2n, VX, ndim)
  call vect_from_car_2_sph (VX, intersection_sph, ndim)
  !
  call arc_len (P1_sph, P2_sph, s1, ndim)
  call arc_len (intersection_sph, P1_sph, sa, ndim)
  call arc_len (intersection_sph, P2_sph, sb, ndim)
  !
  call arc_len (P3_sph, P4_sph, s2, ndim)
  call arc_len (intersection_sph, P3_sph, sc, ndim)
  call arc_len (intersection_sph, P4_sph, sd, ndim)
  !
  positive = (abs(sa+sb-s1).lt.tol) .AND. (abs(sc+sd-s2).lt.tol)
  ...
  !
!
!!--- only for arc < pi/2,  has potential problems ----
!!
!  call XY_dot (VX, V1, ndim, ndim, a)
!  call XY_dot (VX, V2, ndim, ndim, b)
!  call XY_dot (VX, V3, ndim, ndim, c)
!  call XY_dot (VX, V4, ndim, ndim, d)
!  ! bc if two seg meet at end, dot_prod == 0
!  positive = (a.gt.tol) .AND. (b.gt.tol) .AND. (c.gt.tol) .AND. (d.gt.tol)
!  if ( .NOT. positive ) then
!     VX(1:3) = - VX(1:3)
!  endif
!
  return
end subroutine two_great_circ_intersection
...
subroutine two_great_circ_intersection_car (V1, V2, V3, V4, intersection_car, ndim)
  use kinds, only: rt
  implicit none
  integer ,intent(in) :: ndim
  real(rt),intent(in) :: V1(ndim), V2(ndim), V3(ndim), V4(ndim)
  real(rt),intent(out):: intersection_car(ndim)
  !
  ! cartesian coord
  real(rt)  ::  V1n(3), V2n(3), VX(3)
  real(rt)  ::  a, b, c, d
  ...
  call XY_cross (V3, V4, V2n, ndim)
  call XY_cross_unit (V1n, V2n, VX, ndim)
  intersection_car(1:ndim) = VX(1:ndim)
  !
  call arc_len_vector_version (V1, V2, s1, ndim)
  call arc_len_vector_version (VX, V1, sa, ndim)
  call arc_len_vector_version (VX, V2, sb, ndim)
  !
  call arc_len_vector_version (V3, V4, s2, ndim)
  call arc_len_vector_version (VX, V3, sc, ndim)
  call arc_len_vector_version (VX, V4, sd, ndim)
  !
  positive = (abs(sa+sb-s1).lt.tol) .AND. (abs(sc+sd-s2).lt.tol)
  if ( .NOT. positive ) then
     VX(1:3) = - VX(1:3)
  ...
  !
  return
end subroutine two_great_circ_intersection_car
...
FUNCTION plgndr(l,m,x)
  ...
  REAL (rt) :: plgndr,x
  INTEGER i,ll
  REAL (rt) :: fact,pll,pmm,pmmp1,somx2
  if(m.lt.0.or.m.gt.l.or.abs(x).gt.1.) STOP 'function plgndr: assoicated Legendre fails'
  pmm=1.d0
  if(m.gt.0) then
...
FUNCTION Y_lm_sph_harm (l,m,theta,phi)
  use kinds, only: rt
  implicit none
  COMPLEX (rt) :: Y_lm_sph_harm
  INTEGER, intent(in)  :: l,m
  REAL (rt) , intent(in)  :: theta, phi
  !
  ! loc
  REAL (rt) :: x, fact, pi
  REAL (rt) :: A, B, t
  !
  INTEGER, external :: Factorial            ! External function
  REAL(rt),external :: plgndr               ! External function
  !
  if(m.lt.0.or.m.gt.l) STOP 'function Y_lm_sph_harm wrong'
  !
  pi =  4.d0 * atan( 1.d0 ) 
  x  =  sin (theta)            !  cos (theta)
  t  =  dble ( Factorial (l-m) / Factorial (l+m) )
  fact= sqrt ( (2*l+1) / (4.d0 * pi) * t ) 
  A  =  fact * plgndr(l,m,x) * cos( m * phi )
  B  =  fact * plgndr(l,m,x) * sin( m * phi )
  Y_lm_sph_harm = CMPLX(A, B)  
  return
END FUNCTION Y_lm_sph_harm
      ...
      EXTERNAL func
!!      EXTERNAL linmin, dfunc   !     YGYU add
...
!C  (C) Copr. 1986-92 Numerical Recipes Software @1-.
      ...
      do 11 iter=1,ITMAX
        ...
        if(abs(e).gt.tol1) then
          ...
          etemp=e
          e=d
          if(abs(p).ge.abs(0.5*q*etemp).or.p.le.q*(a-x).or.p.ge.q*(b-x)) then
             goto 1
          endif
          d=p/q
          u=x+d
...
!!
!!----  YGYU  these subroutines are pushed to grid.F90 
!!      because (i)  module crt_linear_sp is in cntl/module_variables.F90
!!              (ii) linmin, f1dim, func, dfunc in grid.F90 
!!

SUBROUTINE linmin(p,xi,n,fret)
  use crt_linear_sp, only : ncom, pcom, xicom
  implicit none        
  INTEGER n,NMAX
  REAL*8  fret,p(n),xi(n),TOL
  PARAMETER (NMAX=50,TOL=1.d-5)    ! TOL=1.d-4 default
!CU    USES brent,f1dim,mnbrak
  INTEGER j
  REAL*8  ax,bx,fa,fb,fx,xmin,xx,brent
  EXTERNAL f1dim
!!  ncom=n
  do 11 j=1,n
     pcom(j)=p(j)
...
!#ifdef CG_test
!!
!!   F =  sum_alpha ( x_a - P0_vec(ia))^2  +  lambda * ( sum (x(ia)^2) -1 )
!!  DF =  2 (x_a - P0_vec(ia)) - 2 * lambda * x(ia)
!! 
!! func = the least square mini  function
!!
!function func (p)
!  use crt_linear_sp, only : ncom
!  use gc_LS_tensor, only  : P0_vec
!  implicit none
!  real*8, intent (in)   :: p(ncom)
!  real*8  :: func
!  ! local
!  integer :: ia, ib
!  real*8  :: d_ab, U, s, t
...
!  t=0.d0
!  do ia=1, 3
!     s = s + ( p(ia) - P0_vec(ia) )** 2.d0
!     t = t +   p(ia)*p(ia)
!  enddo
!  func = s + p(4) * (t-1.d0)
!  write(6,'(/2x,a)') 'inside CG_test  func'
!  write(6,'(2x,a,6f12.3)') 'p(1:4)', p(1:4)
!  write(6,'(2x,a,6f12.3)') 'P0_vec(1:3)', P0_vec(1:3)
!  write(6,'(2x,a,6f12.3)') 'func', func
!  return
!end function func
!
!
...
!  implicit none
!  real*8, intent (in)   :: p(ncom)
!  real*8, intent (out)  :: xi(ncom)
!  !loc
!  integer :: ia
!  real*8  :: s
!  s = 0.d0
!  do ia=1, 3
!     xi(ia) = 2.d0 * ( ( p(ia) - P0_vec(ia) ) + p(4)*p(ia) )
!     s = s + p(ia)*p(ia)
!  enddo
!  xi(4) = s - 1.d0
!  write(6,'(/2x,a)') 'inside CG_test  dfunc'
!  write(6,'(2x,a,6f12.3)') 'p(1:4) ', p(1:4)
!  write(6,'(2x,a,6f12.3)') 'xi(1:4)', xi(1:4)
!  return
...
!function func (p)
!  use crt_linear_sp, only : ncom
!  use gc_LS_tensor, only  : Mab_F, x0bar_tensor
!  implicit none
!  real*8, intent (in)   :: p(ncom)
!  real*8  :: func
!  ! local
...
!           d_ab = 0.d0
!        endif
!        U = Mab_F(ia,ib) + 2.d0 * x0bar_tensor(ia) * p(ib) &
!             - d_ab + p(ia) * p(ib)        
!        s = s + U*U
!     enddo
!  enddo
...
!  real*8, intent (out)  :: xi(ncom)
!  !loc
!  integer :: ia, ib, ig     ! a=alph, b=beta, g=gamma
!  real*8  :: s, y
!  real*8  :: d_ab, d_bg, d_ag   
!  ! del_alp_beta, delta_bet_gamma,  delta_alpha_gamma
!  real*8  :: U, V
!  !
!  !-- equation from full LS in GC calc
!  !
!  do ig=1, 3        ! outer loop 
!     !
!     !-- fixed gamma
!     !
!     s = ( p(ig) - x0bar_tensor(ig) ) + p(4) * p(ig)   ! 1st term + 3rd term
!     do ia=1, 3        !  ncom = 4
!        if ( ia.EQ.ig ) then
!           d_ag = 1.d0
!        else
...
!           U = Mab_F(ia,ib) + 2.d0 * x0bar_tensor(ia) * p(ib) &
!                - d_ab + p(ia) * p(ib)
!           V = (2.d0 * x0bar_tensor(ia) + p(ia)) * d_bg + p(ib) * d_ag
!           s = s + U*V      ! sum   2nd term
!        enddo
!     enddo
!     xi(ig) = 2.d0 * s
!  enddo
...
!!
!!-- test ALM:  Bertkesak book, page 121
!!   use Igor Griva book: Linear and non-linear optimization
!!
!!
!
!subroutine  ALM_example (p,n,ftol,iter,fret)
!  use ALM_mod, only : lamk, ck
!  implicit none
!  !   JMAX: Newton-Raphson, NMAX= ndim_max, ikMAX= max lam_k 
!  integer, parameter :: KMAX=100, JMAX=20  
...
!  real*8 :: gfunc, Lfunc
!  EXTERNAL gfunc, Lfunc
!  !
!!CU    USES dfunc,func,linmin
!  INTEGER:: LDA, LDB
!  INTEGER:: j, ik, ia
!  real*8 :: xj(n)          !  alias for p(n)
!  REAL*8 :: xi(n),Y(n)
!  real*8 :: ddA(n,n), ddA_inv(n,n)
!  real*8 :: xacc, L2_norm
!  real*8 :: s
!  !
!  ! minimize    1/2 * x_1 ** 2 + 1/6 * x_2 ** 2
!  ! subject to  x1 + x2 = 1
!  !
!  ! f = 1/2 * x1**2 + 1/6 * x2**2
!  ! g = x1 + x2 -1
!  ! n = 2 
!  ! A = f - lam(k)*g + 1/2 *ck* g*g 
!  ! L = f - lam(k)*g
!  ! 
!  
!  LDA=n; LDB=n
...
!  do ik=1, KMAX
!
!     ! s1: check L2_norm ( dL func )
!     call L2norm_dLfunc (xj, L2_norm)
!     write(6, '(2x,a,f20.14,5x,E10.5)') 'L2_norm, ftol=', L2_norm, ftol
!     s=abs(gfunc(xj))
!     if (s .LT. ftol  .AND. L2_norm .LT. ftol) then
!        goto 200    ! code success
!     endif
!
!!     if (L2_norm .LT. ftol) then
!!        goto 200    ! code success
!!     endif
!
!     !
!     ! s2: unconstrained sub-problem, compute xkp1 to minimize A 
!     !      Newton-Rapson     
!     !      x_j+1 = x_j -  (ddA)^-1 *  dA
!     do j=1, JMAX
!
!        call ddAfunc (xj, ddA)
!        call dAfunc  (xj, xi)
!        call matrix_inv ( ddA, ddA_inv, n, LDA)
!        call AX_mult(ddA_inv, xi, Y, n, n, LDA, LDB)
!        
!        write(6, '(2x,a,2x,4i5)') 'ik, j', ik, j     
!        write(6, '(2x,a)') 'xj, lamk, ck'
!        write(6, '(2x,2f12.3)') xj(1:n)
!        write(6, '(2x,2f12.3)') lamk, ck
!
!        write(6, '(2x,a)') 'ddA'
!        write(6, '(2x,2f12.2)') ddA(1:n,1:n) 
!        write(6, '(2x,a)') 'ddA_inv'
!        write(6, '(2x,2f12.2)') ddA_inv(1:n,1:n) 
!        write(6, '(2x,a)') 'xi_A'
!        write(6, '(2x,2f12.7)') xi(1:n)
!        write(6, '(2x,a)') 'Y'
!        write(6, '(2x,2f12.2)') Y(1:n)
!        write(6,*)
!
!
!        s=0.d0
!        do ia=1,n
!           xj(ia)= xj(ia) - Y(ia)
!           s=s+xi(ia)*xi(ia)          ! pick L_2 norm
!        enddo
!        s=sqrt(s)
!        if (s.LT.xacc) then
!           goto 100          ! Newton-Raphson converged
!        endif
!     enddo   ! xj
!100  continue     
!     !
!     ! s3: update lam_(k+1) = lam_k - ck * g(x_k+1)
!     !     update ck
!     !
!     lamk = lamk - ck * gfunc(xj)
!     ck = ck * 4.d0     ! e.g.  ck = 0.1 * 2**k
!
!     write(6, '(2x,a,2i5)') 'ik=', ik
!     write(6, '(2x,a)') 'xj_k,  lamk, ck (k+1)'
!     write(6, '(2x,4f12.3)') xj(1:n), lamk, ck      
!  enddo      ! loop ik
!  !
!200 continue
!  p(1:n)=xj(1:n)
...
!  iter=ik 
!
!  write(6, '(2x,a)') 'ALM_example, final p'
!  write(6, '(2x,4f12.3)') p(1:n)
!
!  return
!end subroutine ALM_example
!
!
!
!subroutine  ALM_solve_GC_on_sphere (p,n,ftol,iter,fret)
!  use ALM_mod, only : lamk, ck
!  use gc_LS_tensor,  only : Mab_F, x0bar_tensor, A_alph, Uab, delta_ij
!
!  implicit none
!  !   JMAX: Newton-Raphson, NMAX= ndim_max, ikMAX= max lam_k 
!  integer, parameter :: KMAX=20, JMAX=300
!  integer, parameter :: ITMAX=200
!  real*8 , parameter :: EPS=1.d-10
!  INTEGER, intent(in) :: n
!  INTEGER, intent(out):: iter
...
!     ! s1: check L2_norm ( dL func )
!     !
!     !-- update Uab as xj changed
!     !
!     do ia=1,n
!        do ib=1, n
!           Uab(ia,ib)=Mab_F(ia,ib)+A_alph(ia)*x0bar_tensor(ib)- &
!                delta_ij(ia,ib)+x0bar_tensor(ia)*x0bar_tensor(ib)
!        enddo
!     enddo
!     call L2norm_dLfunc (xj, L2_norm)
!     write(6, '(2x,a,f20.14,5x,E10.5)') 'L2_norm, ftol=', L2_norm, ftol
!     s=abs(gfunc(xj))                                   ! bc my Func is Y^2 --> 0 
!     if (s .LT. ftol  .AND. L2_norm .LT. ftol) then
!        goto 200    ! code success
!     endif
!     !
...
!        write(6, '(2x,a,2x,4i5)') 'ik, j', ik, j     
!        write(6, '(2x,a)') 'xj, lamk, ck'
!        write(6, '(2x,3f12.3)') xj(1:n)
!        write(6, '(2x,3f12.3)') lamk, ck
!
!        write(6, '(2x,a)') 'ddA'
!        write(6, '(2x,3f12.2)') ddA(1:n,1:n) 
!        write(6, '(2x,a)') 'ddA_inv'
!        write(6, '(2x,3f12.2)') ddA_inv(1:n,1:n) 
!        write(6, '(2x,a)') 'xi_A'
!        write(6, '(2x,3f12.7)') xi(1:n)
!        write(6, '(2x,a)') 'Y'
!        write(6, '(2x,3f12.2)') Y(1:n)
!        write(6,*)
!
!        s=0.d0
!        do ia=1,n
...
!     !
!     lamk = lamk - ck * gfunc(xj)
!     ck = ck * 6.d0     ! e.g.  ck = 0.1 * 2**k
!
!     write(6, '(2x,a,2i5)') 'ik=', ik
!     write(6, '(2x,a)') 'xj_k,  lamk, ck (k+1)'
!     write(6, '(2x,10f12.3)') xj(1:n), lamk, ck      
!  enddo      ! loop ik
!  !
!200 continue
!  p(1:n)=xj(1:n)
...
!  real*8  :: func
!  ! f = 1/2 * x1**2 + 1/6 * x2**2
!  func=0.5d0*p(1)*p(1)+1.d0/6.d0*p(2)*p(2)
!end function func
!
!!
!!function gfunc (p)
...
!  !  L = f - lam*g
!  !
!  Lfunc=func(p)-lamk*gfunc(p)
!  return
!end function Lfunc
!
!
!subroutine dfunc (p, xi_f)
!  use crt_linear_sp, only : ncom
!  implicit none
!  real*8, intent (in)   :: p(ncom)
!  real*8, intent (out)  :: xi_f(ncom)
!  real*8 :: v(ncom)
!  ! dF 
!  v(1)=p(1)
!  v(2)=p(2)/3.d0
...
!
!
!!subroutine dgfunc (p, xi_g)
!!  use crt_linear_sp, only : ncom
!!  implicit none
!!  real*8, intent (in)   :: p(ncom)
!!  real*8, intent (out)  :: xi_g(ncom)
!!  real*8 :: v(ncom)
!!  ! dG 
!!  v(1)=2.d0*p(1)
!!  v(2)=2.d0*p(2)
...
!  external   gfunc
!  !
!  real*8  :: xi_f(ncom), xi_g(ncom), v(ncom)
!  integer :: i
!  !
!  !  A = f - lam*g + 1/2*ck*g*g
!  !
!  ! dA = df - lam*dg + ck*g*dG
!  !
!  !
!  call dfunc (p,xi_f)
!  call dgfunc(p,xi_g)
!  ! dA
!  do i=1,2
!     v(i)= xi_f(i) - lamk*xi_g(i) + ck*gfunc(p)*xi_g(i)
!  enddo
!  xi(1:2)=v(1:2)
!
!  write(6, '(2x,a)')  'gfunc, lamk, ck'
!  write(6,'(2x,3f12.2)')  gfunc(p), lamk, ck
!  write(6, '(2x,a)')  'p, xi_f, xi_g, xi_A'
!  write(6,'(2x,3f12.2)')  p, xi_f, xi_g, xi
!
!  return
!end subroutine dAfunc
!
!
!!subroutine ddAfunc (p, ddA)
!!  use crt_linear_sp, only : ncom
!!  use ALM_mod, only : lamk, ck  
!!  implicit none
!!  real*8, intent (in)   :: p(ncom)
!!  real*8, intent (out)  :: ddA(ncom,ncom)
!!  real*8 :: gfunc
!!  external   gfunc
!!  !
!!  real*8 :: ddF(ncom,ncom), ddG(ncom, ncom)
!!  real*8 :: xi_g(ncom), v(ncom)
!!  integer:: i, j
!!  !
!!  !  A = f - lam*g + 1/2*ck*g*g
!!  !
!!  !ddA = ddf - lam*ddG + ck * ( dG(i) dG(j) + g * ddG(i,j) )
!!  !
!!  ! ddF=  1   0        ddG= 2 0
!!  !       0  1/3            0 2
!!  !
!!  ddF(1,1)=1.d0; ddF(2,2)=1.d0/3.d0
!!  ddF(1,2)=0.d0; ddF(2,1)=0.d0
!!  ddG(1,1)=2.d0; ddG(2,2)=2.d0
!!  ddG(1,2)=0.d0; ddG(2,1)=0.d0
!!  ! dG
!!  call dgfunc(p,xi_g)
!!  !
!!  do i=1,2
!!     do j=1,2
!!        ddA(i,j)= ddF(i,j)-lamk*ddG(i,j)+&
!!             ck*(xi_g(i)*xi_g(j)+gfunc(p)*ddG(i,j))
!!     enddo
!!  enddo
!!  return
!!end subroutine ddAfunc
!
!
!subroutine ddAfunc (p, ddA)
!  use crt_linear_sp, only : ncom
...
!
!
!subroutine L2norm_dLfunc (p, L2_norm)
!  use crt_linear_sp, only : ncom
!  use ALM_mod, only : lamk
!  implicit none
!  real*8, intent (in)   :: p(ncom)
!  real*8, intent (out)  :: L2_norm
!  real*8  :: xi_f(ncom), xi_g(ncom), v(ncom), s
!  integer :: i
!  !
!  !  L = f - lam*g
!  !  A = f - lam*g + 1/2*ck*g*g
!  ! dA = df - lam*dg + 1/2*ck*g*dG
!  ! dL = df - lam*dg
!  !
!  ! dF, dG
!  call dfunc (p,xi_f)
...
!  !
!  return
!end subroutine L2norm_dLfunc
!
!#endif
!

#define ALM_GC_tensor
#ifdef ALM_GC_tensor

function func (p)
  use crt_linear_sp, only : ncom
  use gc_LS_tensor, only  : Mab_F, x0bar_tensor, Uab, A_alph, delta_ij
  implicit none
  real*8, intent (in)   :: p(ncom)
  real*8  :: func, s, delta, Uabx, A_alphx
  integer :: i, j 
  ! f = \sum (x_a - x_a^0 ^bar) ^2 + Uab * Uab
  s=0.d0
  do i=1, 3
     s=s+(p(i) - x0bar_tensor(i))**2.d0
  enddo
  !
  do i=1, 3
!!     A_alph(i) = 2.d0*x0bar_tensor(i)
!!     A_alphx = 2.d0*x0bar_tensor(i)
     do j=1, 3
!!        Uab(i,j) = Mab_F(i,j) - delta + A_alph(i)*p(j) + p(i)*p(j)
!!        Uabx = Mab_F(i,j) - delta_ij(i,j) + A_alphx*p(j) + p(i)*p(j)
        s=s+Uab(i,j)*Uab(i,j)
     enddo
  enddo
  func=s
...
function gfunc (p)
  ...
  real*8  :: gfunc
  ! g = x1**2 + x2**2 + x3*2- 1
  gfunc=p(1)*p(1)+p(2)*p(2)+p(3)*p(3)-1.d0
end function gfunc
...
subroutine dfunc (p, xi_f)
  use crt_linear_sp, only : ncom
  use gc_LS_tensor, only  : x0bar_tensor, Uab, A_alph
  implicit none
  real*8, intent (in)   :: p(ncom)
  real*8, intent (out)  :: xi_f(ncom)
  real*8 :: v(ncom), s
  integer:: i, ia
  !
  ! \p F / \p xj = 2*(xj - xj^0) + 2 Uaj (A_alph + x_a ) + 2 Ujb x_b
  !
  do i=1, 3
     s = p(i) - x0bar_tensor(i) 
     do ia=1, 3
        s = s + Uab(ia,i) * (A_alph(ia) + p(ia))
     enddo
     do ia=1, 3
        s = s + Uab(i,ia) * p(ia)
     enddo
...
subroutine ddAfunc (p, ddA)
  use crt_linear_sp, only : ncom
  use ALM_mod, only : lamk, ck  
  use gc_LS_tensor, only  : x0bar_tensor, Uab, A_alph, delta_ij
  implicit none
  real*8, intent (in)   :: p(ncom)
  real*8, intent (out)  :: ddA(ncom,ncom)
  real*8 :: gfunc
  ...
  do i=1,3
     do j=1,3
        s= delta_ij(i,j)+Uab(i,j)+Uab(j,i)+ &
           (A_alph(i)+x0bar_tensor(i))*p(j)+ &
           (A_alph(j)+x0bar_tensor(j))*p(i)
        t=0.d0
        do ia=1,3
           t=t+(A_alph(ia)+x0bar_tensor(ia))**2.d0+&
                x0bar_tensor(ia)**2.d0
        enddo
        s= s + t * delta_ij(i,j)
        ddF(i,j)= 2.d0*s
     enddo
